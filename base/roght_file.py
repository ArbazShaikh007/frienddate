...
from base.user.models import token_required, FriendRequest, User, DateRequest, TagFriends, \
    ChatMute, Notification, Block, TblCountries, TblStates, Follow, Feed, FeedLike, FeedComments, NewNotification, \
    LikeRecommendation
from base.user.queryset import insert_data, delete_frnd_req
from base.admin.models import Category, Faqs, ThingsCategory, CategoryQue, CategoryAns, QuestionsCategory, Buttons
from base import db
from base.community.models import SavedCommunity, CreatedThingsCommunity, CreatedCommunity, CommunityPost, \
    ThingsRecommendation, PlacesRecommendation
...
def like_recommendation(active_user):
    ...
    if type == 'things':
        things_data = ThingsRecommendation.query.filter_by(user_id=user_id, community_id=community_id,
...
def feed_comment_list(active_user):
    ...
    comments_data = FeedComments.query.filter_by(feed_id=feed_id).paginate(page=page, per_page=per_page,
                                                                           error_out=False)
    comments_count = FeedComments.query.filter_by(feed_id=feed_id).count()
    ...
    return jsonify(
        {'status': 1, 'messege': 'Success', 'comment_list': comments_list, 'pagination_info': pagination_info,
...
def add_comment_feed(active_user):
    ...
    add_feed_comment_data = FeedComments(text=comment_text, user_id=active_user.id, feed_id=feed_id,
...
def like_feed(active_user):
    ...
    if check_liked:
        ...
        if active_user.id != feed_data.feed_id.id:
            ...
            add_notification = NewNotification(feed_id=feed_data.id, title=title, message=msg, by_id=active_user.id,
                                               to_id=feed_data.feed_id.id,
...
def users_review_list(active_user):
    ...
    if feed_data.items:
        feed_list = [i.as_dict(active_user.id) for i in feed_data.items]
        return jsonify(
            {'status': 1, 'messege': 'Success', 'review_list': feed_list, 'pagination_info': pagination_info})
    else:
        pagination_info = {
            "current_page": 1,
            "has_next": False,
        ...
        return jsonify({'status': 1, 'messege': 'Sorry no reviews share by user', 'review_list': [],
                        'pagination_info': pagination_info})
...
def feed_page(active_user):
    ...
    if not follow_data:
        ...
        return jsonify({'status': 1, 'messege': 'First you need to follow someone to see their activity in feed page',
                        'feed_list': [], 'pagination_info': pagination_info})

    followed_list = [i.to_id for i in follow_data]

    feed_data = Feed.query.filter(Feed.user_id != active_user.id, Feed.user_id.in_(followed_list)).order_by(
        Feed.id.desc()).paginate(page=page, per_page=per_page, error_out=False)
    ...
    if feed_data.items:
        ...
        return jsonify(
            {'status': 1, 'messege': 'Success', 'notification_count': str(notification_count), 'feed_list': feed_list,
             'pagination_info': pagination_info})
    else:
        pagination_info = {
            "current_page": 1,
        ...
        return jsonify(
            {'status': 1, 'messege': 'Sorry no feeds share by anyones', 'notification_count': str(notification_count),
             'feed_list': [], 'pagination_info': pagination_info})
...
def add_activity_post(active_user):
    ...
    print('content_media_typeeeeeeeeeeeeeeeeeeeeeeeeee', content_media_type)
    ...
    add_feed_data = Feed(type=type, text=text, thumbnail_path=thumbnail_path, video_path=video_url,
...
def followers_list(active_user):
    ...
    get_followers_data = Follow.query.filter(Follow.to_id == user_id).order_by(Follow.id.desc()).paginate(page=page,
                                                                                                          per_page=per_page,
                                                                                                          error_out=False)
    ...
    return jsonify(
        {'status': 1, 'messege': 'Success', 'followes_list': followes_list, 'pagination_info': pagination_info})
...
def user_recommendation_category(active_user):
    ...
    print('category_object_listtttttttttttttttttttttttttttttt', category_object_list)
...
def category_list(active_user):
    ...
    if tab == 0:
        ...
        category_list = [category.as_dict(count) for category, count in categories_with_count]
...
def matches_category_vice(active_user):
    ...
    is_follow = 0
    follow_data = Follow.query.filter_by(by_id=active_user.id, to_id=user_id).first()
    follow_count = Follow.query.filter_by(to_id=user_id).count()
    if follow_data:
        is_follow = 1
    ...
    badge = ''
    if only_user.badge_name is not None:
        if only_user.badge_name == "I'll Buy Us Coffee":
            badge = "?"
        if specific_response.badge_name == "I'll Buy Us Food":
            badge = "??"
        if specific_response.badge_name == "Activity Badge":
    ...
    if not friend_request:
        return jsonify({'status': 1, 'follow_count': str(follow_count), 'is_follow': is_follow, 'is_friends': 0,
                        'is_datereq': bool(already_send), 'user_data': user_dict,
        ...
        return jsonify({'status': 1, 'follow_count': str(follow_count), 'is_follow': is_follow, 'is_friends': 2,
                        'is_datereq': bool(already_send), 'user_data': user_dict,
                        'matches': dict_list1, 'unmatches': ['this is static value'], 'filter': filter_text or 0,
                        'badge': badge,
                        'is_block': is_block, "is_subscribed": is_subscribed, "description_box": description_box,
        ...
        return jsonify({'status': 1, 'follow_count': str(follow_count), 'is_follow': is_follow, 'is_friends': 1,
                        'is_datereq': bool(already_send), 'user_data': user_dict,
...
def featured_page(active_user):
    ...
    if user_data.items:
        ...
        return jsonify(
            {'status': 1, 'data': [], 'messege': 'No result', 'pagination_info': pagination_info})
...
def matches_community_vice(active_user):
    if request.method == 'POST':
        ...
        if len(response_list) > 0:
            ...
            return jsonify({'status': 1, 'pagination_info': pagination_info, 'data': [],
...
def give_answer(active_user):
    ...
    print('dataaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', main_data)
    if data:
        for i in main_data:
