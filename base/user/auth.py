...
from base.user.models import SelectedCategory, Block, Report, Notification, FriendRequest, SelectedCategory,Follow
...
def register():
    if request.method == 'POST':
        ...
        phonenumber = request.json.get('userPhonenumber')
        country_code = request.json.get('country_code')
        age_verify = request.json.get('ageVerify')
        ...
        if not user:
            ...
            user_data = User(fullname=fullname, email=email, country_code=country_code, device_token=device_token,
                             device_type=device_type,
                             phonenumber=phonenumber, password=hash_password, image_path=image_path,
                             created_time=datetime.utcnow(), age=date_obj, is_18plus=age_verify)
            if age_verify == True:
                ...
                cat_id_ls = []
                for i in x:
                    cat_id_ls.append(str(i.id))
                y = SelectedCategory(user_id=user_data.id, category_id=','.join(cat_id_ls))
                db.session.add(y)
                db.session.commit()

                token = jwt.encode({'id': user_data.id, 'exp': datetime.utcnow() + timedelta(days=365)},
                                   '192b9bdd22ab9ed4d12e236c78afcb9a393ec15f71bbf5dc987d54727823bcbf')
                return jsonify({'status': 1, 'message': 'success', 'data': user_data.as_dict(), 'token': token})
            else:
                return jsonify({'status': 1, 'message': 'please confirm your age'})
        ...
        if check_email and check_email.check_password(password):
            ...
            for i in none_exitsting_device_token:
                if i.device_token == device_token:
                    if i.is_subscription == True:
                        check_email.is_subscription = True
                        check_email.subscription_start_time = i.subscription_start_time
                        check_email.subscription_end_time = i.subscription_end_time
                        check_email.subscription_price = i.subscription_price
                        check_email.product_id = i.product_id
                        check_email.transaction_id = i.transaction_id
                        check_email.purchase_date = i.purchase_date
                    i.device_token = None
                    i.device_type = None
                    db.session.commit()
...
@user_auth.route("/token_verification")
@token_required
...
@user_auth.route('/change_pwd', methods=['POST'])
@token_required
def change_password(active_user):
    # old_pwd = request.json.get('oldPassword')
    ...
    # if active_user and active_user.check_password(old_pwd):
    if new_pwd == confirm_pwd:
        ...
        # else:
        # return jsonify({'status': 0, 'message': 'Wrong Old Password'})


@user_auth.route('/user_update', methods=['POST', 'GET'])
@token_required
def user_update(active_user):
    ...
    if active_user.is_subscription == True:

        if int(active_user.subscription_start_time) <= current_timestamp_ms <= int(active_user.subscription_end_time):
            pass
        else:
    ...
    if request.method == 'POST':
        ...
        # id = request.form.getlist('category_id')
        print('smokeeeeeeeeeeeeeeeeeeeeeeeeeeeeee', id)
        ...
        if user_image:
            if active_user.image_name != user_image:

                for friend in friend_ls:
                    if active_user.relationship_status != relationship_status:
                        if friend.relationship_status_change == True:

                            title = 'Relationship Status Changed'
                            # image_url = f'{COMMON_URL}{active_user.image_path[2:]}{active_user.image_name}'
                            msg = f'{active_user.fullname} Change Their Relationship Status'
                            add_notification = Notification(title=title, messege=msg, by_id=active_user.id,
                                                            to_id=friend.id, is_read=False,
                                                            created_time=datetime.utcnow(), post_id=None,
                                                            community_id=None, page='user')
                    ...
                    if friend.profile_pic == True:
                        title = 'Profile Picture Changed'
                        # image_url = f'{COMMON_URL}{active_user.image_path[2:]}{active_user.image_name}'
                        msg = f'{active_user.fullname} Change Their Profile Picture'
            ...
            image_path = os.path.join(UPLOAD_FOLDER)
            user_image.save(os.path.join(image_path, image_name))
            image_path = image_path.replace("base", "..")

            user_data = User(description_box=description_box, id=active_user.id, email=active_user.email,
            ...
            if active_user.relationship_status != relationship_status:
                for friends in friend_ls:
                    if friends.relationship_status_change == True:

                        title = 'Relationship Status Changed'
                        # image_url = f'{COMMON_URL}{active_user.image_path[2:]}{active_user.image_name}'
                        msg = f'{active_user.fullname} Change Their Relationship Status'
                        add_notification = Notification(title=title, messege=msg, by_id=active_user.id,
                                                        to_id=friends.id, is_read=False, created_time=datetime.utcnow(),
                                                        post_id=None, community_id=None, page='user')
                        db.session.add(add_notification)
                        db.session.commit()
                        # if friend.device_token:
                        notification = push_notification(device_token=friends.device_token, title=title, msg=msg,
                                                         image_url=None, device_type=friends.device_type)
                    else:
                        title = 'Relationship Status Changed'
                        msg = f'{active_user.fullname} Change Their Relationship Status'
                        add_notification = Notification(title=title, messege=msg, by_id=active_user.id,
        ...
        users_data = {'id': str(active_user.id),
                      ...
                      'user_image': COMMON_URL + active_user.image_path[2:] + active_user.image_name,
        ...
        if len(id) > 0 and id[0] != "":
            ...
            if not len(active_user.category_id) > 0:
                ...
                id = list(set(idss))
                category_ids = [item for item in id if item not in category_idss]
                # join_id = ",".join(id)
                # if len(category_ids)>0:
                # all_list = category_ids+category_idss
                # x.category_id = ",".join(all_list)
                # x.id = x.id
                # db.session.commit()
                ...
                # ob = SelectedCategory(id=x.id, category_id=join_id, user_id=active_user.id)
                # update_data(ob)
    ...
    return jsonify({'status': 1, 'data': total_data})


@user_auth.route('/double_verification', methods=['POST'])
@token_required
...
@user_auth.route("/user_to_user/block", methods=['POST'])
@token_required
...
@user_auth.route("/block_list", methods=['GET'])
@token_required
def block_list(active_user):
    ...
    for i in active_user.user_id:
        ...
        blocked_dict = {
            ...
            'user_image': COMMON_URL + user_info.image_path[2:] + user_info.image_name}
...
@user_auth.route("/user/report", methods=['POST'])
@token_required
...
@user_auth.route("/user/forget_password", methods=['POST'])
...
@user_auth.route("/user/new_password", methods=['POST'])
@token_required
...
@user_auth.route("/subscription", methods=['POST'])
@token_required
...
@user_auth.route('/add_image_s', methods=['GET', 'POST'])
def add_image_s():
