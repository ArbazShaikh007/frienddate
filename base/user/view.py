...
from base.user.models import token_required, FriendRequest, User, SelectedCategory, DateRequest, Report, TagFriends, \
    ChatMute, Notification, Block, TblCountries, TblStates,Follow
...
@user_view.route('/homepage_verify_token', methods=['GET'])
@token_required
def homepage_verify_token(active_user):
    # receipt_data = request.json.get('receipt')
    # if receipt_data:
    ...
    # entry = receipt_info['latest_receipt_info'][0]
...
@user_view.route('/homepage', methods=['POST'])
@token_required
...
@user_view.route('/homepage_filter', methods=['GET', 'POST'])
@token_required
def homepage_filter(active_user):
    ...
    if request.method == 'POST':
        ...
        print('age Strattttttttttttttttttttt', age_start)
        age_end = request.json.get('age_end')
        print('age Endsssssssssssssssssssss', type(age_end))
...
@user_view.route('/category_list', methods=['GET', 'POST'])
@token_required
...
@user_view.route('/users_list', methods=['GET'])
@token_required
...
@user_view.route('/send_friend_req', methods=['POST'])
@token_required
...
@user_view.route('/req_list', methods=['GET', 'POST'])
@token_required
...
@user_view.route('/req_action', methods=['POST'])
@token_required
...
@user_view.route('/friends_list', methods=['POST'])
@token_required
...
@user_view.route('/friends_list_id', methods=['GET', 'POST'])
@token_required
...
@user_view.route('/get_category', methods=['GET', 'POST'])
@token_required
...
@user_view.route('/view_profile', methods=['POST'])
@token_required
...
@user_view.route("/user/delete", methods=['GET', 'POST'])
@token_required
...
@user_view.route("/matches/category_vise", methods=['POST'])
@token_required
def matches_category_vice(active_user):
    ...
    if only_user:
        if only_user.is_subscription == True:
            current_timestamp_ms = int(datetime.now().timestamp() * 1000)
            if int(only_user.subscription_start_time) <= current_timestamp_ms <= int(only_user.subscription_end_time):
                pass
        else:
            only_user.is_subscription = False
            only_user.subscription_start_time = None
            only_user.subscription_end_time = None
            only_user.subscription_price = None
            only_user.product_id = None
            only_user.transaction_id = None
            only_user.purchase_date = None
            db.session.commit()
    ...
    user_dict = {'user_name': '@' + only_user.fullname,
                 'user_image': COMMON_URL + only_user.image_path[2:] + only_user.image_name, }
...
@user_view.route("/get/terms_conditions", methods=['GET'])
def get_terms_conditions():
    x = terms_condition(1)
    return jsonify({'status': 1, 'content': x.as_dict()})


@user_view.route("/get/privacy_policy", methods=['GET'])
def get_privacy_policy():
    x = terms_condition(2)
    return jsonify({'status': 1, 'content': x.as_dict()})


@user_view.route("/get/news", methods=['GET'])
@token_required
...
@user_view.route("/get/how_to_use", methods=['GET'])
@token_required
...
@user_view.route("/get/information", methods=['GET'])
@token_required
...
@user_view.route("/get/stores", methods=['GET'])
@token_required
...
@user_view.route("/get/brands_deals", methods=['GET'])
@token_required
...
@user_view.route("/get/faq", methods=['GET'])
@token_required
...
@user_view.route("/search/user", methods=['GET', 'POST'])
@token_required
                ...
                if i.deleted == False:
                    ...
                    if not block_check:
                        dict = {
                            ...
                            'user_image': COMMON_URL + i.image_path[2:] + i.image_name}
                        ls.append(dict)
...
@user_view.route('/get/tag_friends', methods=['POST'])
@token_required
            ...
            if friend_user.deleted == False:
                if rmove.user_id != friend_user.id:
                    friends_list.append({
                        ...
                        'user_image': COMMON_URL + friend_user.image_path[2:] + friend_user.image_name,
                        'is_tagged': check_tag
                    })
...
@user_view.route('/tag_friends', methods=['GET', 'POST'])
@token_required
...
@user_view.route('/post/mute_unmute', methods=['GET', 'POST'])
@token_required
...
@user_view.route('/community/unsave', methods=['POST'])
@token_required
...
@user_view.route('/matches/community_vice', methods=['POST'])
@token_required
...
@user_view.route('/notification_list', methods=['POST'])
@token_required
...
@user_view.route('/notification_button', methods=['POST'])
@token_required
...
@user_view.route('/get_notification_button', methods=['GET'])
@token_required
...
@user_view.route('/ranking_page', methods=['POST'])
@token_required
...
@user_view.route('/verify-receipt', methods=['POST'])
@token_required
def verify_receipt(active_user):
    ...
    if response.status_code == 200:
        receipt_info = response.json()
        print('receipt_infoooooooooooooooooooooooooooooooooooooooooooooooooooooooooorrrr', receipt_info)
        print('receipt_latestttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt',
              receipt_info['latest_receipt_info'])
        if receipt_info.get('status') == 0 and receipt_info.get('latest_receipt_info'):
            entry1 = receipt_info['latest_receipt_info']
            found = False
            for entry in entry1:
                if entry['product_id'] == 'frienddateMonthly':
                    found = True
                    current_timestamp_ms = int(datetime.now().timestamp() * 1000)
...
@user_view.route('/get/countries', methods=['GET'])
@token_required
def get_countries(active_user):
...
@user_view.route('/get/states', methods=['POST'])
@token_required
def get_states(active_user):
