...
community_create_v2 = Blueprint('community_create_v2', __name__)


@community_create_v2.route('/add_community', methods=['GET', 'POST'])
@token_required
    ...
    if request.method == 'POST':
        ...
        if community_exits:
            ...
            if saved_community_exits:
                ...
                return jsonify({'status': 1, 'data': communit.as_dict(), 'messege': 'Saved'})
...
@community_create_v2.route('/save_community', methods=['POST'])
@token_required
...
@community_create_v2.route('/join_community', methods=['POST'])
@token_required
...
@community_create_v2.route('/my_community', methods=['GET', 'POST'])
@token_required
...
@community_create_v2.route('/community_library', methods=['POST'])
@token_required
...
@community_create_v2.route('/community_list', methods=['POST'])
@token_required
def community_list(active_user):
    filter_text = request.json.get('filter_text')
    print('filter_textttttttttttttttttttttttttttt', filter_text)
    category_id = request.json.get('category_id')
    print('category_iddddddddddddddddddddddddddddddd', category_id)
    page = int(request.json.get('page', 1))  # Default to page 1 if not specified
    print('pageeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee', page)
    per_page = 10  # Number of items per page
    ...
    blocked_user_ids = [block.user_id for block in Block.query.filter_by(blocked_user=active_user.id).all()]
    blocked_by_user_ids = [block.blocked_user for block in Block.query.filter_by(user_id=active_user.id).all()]
    print('blocked_user_idssssssssssssssssssssss', blocked_user_ids)
    print('blocked_by_user_idssssssssssssssssssssss', blocked_by_user_ids)
    ...
    if filter_text == 1:
        query = query.order_by(CreatedCommunity.community_name.desc(),
                               CreatedCommunity.id)  # Primary by name, secondary by ID
    elif filter_text == 2:
        query = query.order_by(CreatedCommunity.id.desc())  # Already unique, no secondary needed
    elif filter_text == 3:
        query = query.order_by(CreatedCommunity.id.asc())  # Already unique, no secondary needed
    elif filter_text == 4:
        query = query.order_by(func.count(SavedCommunity.id).desc(),
                               CreatedCommunity.id)  # Primary by count, secondary by ID
    elif filter_text == 5:
    ...
    if created_data.items:
        ...
        for id, community_name, count in created_data.items:
...
@community_create_v2.route('/community_post', methods=['GET', 'POST'])
@token_required
...
@community_create_v2.route('/like_post', methods=['POST'])
@token_required
...
@community_create_v2.route('/thumsup_post', methods=['POST'])
@token_required
...
@community_create_v2.route('/thumsdown_post', methods=['POST'])
@token_required
...
@community_create_v2.route('/post_comment', methods=['GET', 'POST'])
@token_required
...
@community_create_v2.route('/view/post_comment', methods=['POST'])
@token_required
...
@community_create_v2.route('/post_list', methods=['POST'])
@token_required
...
@community_create_v2.route('/delete_post', methods=['POST'])
@token_required
