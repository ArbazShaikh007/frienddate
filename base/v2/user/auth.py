...
from base.admin.models import Category, Badges, Buddys
from base.user.models import SelectedCategory, Block, Report, Notification, FriendRequest, SelectedCategory
...
user_auth_v2 = Blueprint('user_auth_v2', __name__)


@user_auth_v2.route('/replace_s3', methods=['GET'])
def replace_s3():
...
@user_auth_v2.route('/register', methods=['POST'])
def register():
    if request.method == 'POST':
        ...
        if not user:
            ...
            user_data = User(college=college, about_me=about_me, city=city, state=state, country=country,
            ...
            if age_verify == True:
                ...
                return jsonify({'status': 1, 'messege': 'please confirm your age'})


@user_auth_v2.route('/login', methods=['POST'])
def user_login():
    if request.method == 'POST':
...
@user_auth_v2.route("/token_verification")
@token_required
...
@user_auth_v2.route('/change_pwd', methods=['POST'])
@token_required
...
@user_auth_v2.route('/user_update', methods=['POST', 'GET'])
@token_required
def user_update(active_user):
    ...
    if request.method == 'POST':
        ...
        if user_image:
                    ...
                    if active_user.relationship_status != relationship_status:
                        if friend.relationship_status_change == True:
                            ...
                            add_notification = Notification(title=title, messege=msg, by_id=active_user.id,
                                                            to_id=friends.id, is_read=False,
            ...
            user_data = User(college=college, about_me=about_me, description_box=description_box, id=active_user.id,
        ...
        buddy_data = Buddys.query.all()
        buddy_list = []
        check_buddy_value = False
        if active_user.is_subscription_buddy == True:
            check_buddy_value = True
        if len(buddy_data) > 0:
            for buddy_name in buddy_data:
                buddy_status = False
                if check_buddy_value == True:
                    if buddy_name == active_user.buddy_name:
                        buddy_status = True

                buddy_list.append({'buddy_name': buddy_name.type, 'buddy_status': buddy_status})
        badge_name = ""
        ...
        users_data = {'id': str(active_user.id),
                      ...
                      'is_subscription_buddy': active_user.is_subscription_buddy,
...
@user_auth_v2.route('/double_verification', methods=['POST'])
@token_required
...
@user_auth_v2.route("/user_to_user/block", methods=['POST'])
@token_required
...
@user_auth_v2.route("/block_list", methods=['GET'])
@token_required
...
@user_auth_v2.route("/user/report", methods=['POST'])
@token_required
...
@user_auth_v2.route("/user/forget_password", methods=['POST'])
...
@user_auth_v2.route("/user/new_password", methods=['POST'])
@token_required
...
@user_auth_v2.route("/subscription", methods=['POST'])
@token_required
...
@user_auth_v2.route('/add_image_s', methods=['GET', 'POST'])
def add_image_s():
