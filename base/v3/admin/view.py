...
admin_views_v3 = Blueprint('admin_views_v3', __name__)


@admin_views_v3.route('/view/users', methods=['POST', 'GET'])
@login_required
def view_users():
...
@admin_views_v3.route('/view/featured_list', methods=['POST', 'GET'])
@login_required
...
@admin_views_v3.route('/add_users_featured_page', methods=['GET'])
@login_required
def add_users_featured_page():
    ...
    if user_data:
        if user_data.is_featured == False:
            ...
            db.session.commit()
            if page == 'user_list':
                return redirect(url_for('admin_views_v3.view_users'))
            if page == 'feartured':
                return redirect(url_for('admin_views_v3.view_featured_list'))
...
@admin_views_v3.route('/user/reports', methods=['POST', 'GET'])
@login_required
def user_reports():
    # get_page = request.args.get('page', 1, type=int)
...
@admin_views_v3.route("/user/block/<int:id>", methods=['POST'])
@login_required
def block_user(id):
...
@admin_views_v3.route('/view/terms_conditions', methods=['POST', 'GET'])
@login_required
def terms_conditions():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('admin_views_v3.terms_conditions'))

    return render_template('termsConditions.html', data=current_user, content=x, page='terms', common_path=COMMON_PATH)


@admin_views_v3.route('/view/privacy_policies', methods=['POST', 'GET'])
@login_required
def privacy_policies():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('admin_views_v3.privacy_policies'))

    return render_template('privacyPolicies.html', data=current_user, content=x, page='privacy',
                           common_path=COMMON_PATH)


@admin_views_v3.route('/add/faq', methods=['POST', 'GET'])
@login_required
def add_faq():
    get_page = request.args.get('page', 1, type=int)
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('admin_views_v3.add_faq'))
...
@admin_views_v3.route('/delete_faq/<int:id>', methods=['POST'])
@login_required
def delete_faq(id):
...
@admin_views_v3.route('/faq/update', methods=['POST', 'GET'])
@login_required
def update_faq():
...
@admin_views_v3.route('/how_to_use_friend_date', methods=['POST', 'GET'])
@login_required
def how_to_use_app():
    x = terms_condition(4)
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('admin_views_v3.how_to_use_app'))

    return render_template('howtouseFrienddate.html', data=current_user, content=x, page='app_use',
                           common_path=COMMON_PATH)


@admin_views_v3.route('/information', methods=['POST', 'GET'])
@login_required
def information():
    x = terms_condition(5)
...
@admin_views_v3.route("/user/reports", methods=['GET', 'POST'])
@login_required
def view_reports():
