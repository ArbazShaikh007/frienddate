...
from base.user.models import User, token_required,Follow
...
import boto3
from base.v5.user.view import generate_qr_code_fun
...
user_auth_v5 = Blueprint('user_auth_v5', __name__)


@user_auth_v5.route('/replace_s3', methods=['GET'])
def replace_s3():
...
@user_auth_v5.route('/register', methods=['POST'])
def register():
    if request.method == 'POST':
        ...
        if not gender:
            return jsonify({'status':0, 'messege': 'Gender is required'})
        if not device_type:
            return jsonify({'status':0, 'messege': 'Device token is required'})
        ...
        print('city ', city)
        if not city:
            return jsonify({'status':0, 'messege': 'City is required'})

        state = request.form.get('state')
        print('state ', state)
        if not state:
            return jsonify({'status':0, 'messege': 'State is required'})

        country = request.form.get('country')
        print('country ', country)
        if not country:
            return jsonify({'status': 0, 'messege': 'Country is required'})
        ...
        if not age_verify:
            return jsonify({'status': 0,'messege': 'Please select 18+ checkbox'})
        #
        # if gender == '0':
        #     gender = 'Male'
        # elif gender == '1':
        ...
        has_had_birthday = (today.month, today.day) >= (date_obj.month, date_obj.day)
        actual_age = years_difference if has_had_birthday else years_difference - 1

        if not actual_age >= 18:
            return jsonify({'status': 0,'messege': 'Your age must have 18+'})
        ...
        if not user:
            ...
            user_data = User(age_verify=age_verify,
                               sexuality=sexuality,
            ...
            file_path = generate_qr_code_fun(user_data.id,fullname)

            get_user = User.query.get(user_data.id)

            get_user.qr_code = file_path
            db.session.commit()
...
@user_auth_v5.route('/login', methods=['POST'])
def user_login():
    if request.method == 'POST':
...
@user_auth_v5.route('/social_register', methods=['POST'])
def social_register():
    if request.method == 'POST':
...
@user_auth_v5.route("/update_lat_long", methods=['POST'])
@token_required
def update_lat_long(active_user):
    ...
    if not lat:
        return jsonify({'status': 0,'messege': 'Please provide latitude'})
    if not long:
        return jsonify({'status': 0,'messege': 'Please provide longitude'})
...
@user_auth_v5.route("/token_verification")
@token_required
...
@user_auth_v5.route('/change_pwd', methods=['POST'])
@token_required
...
@user_auth_v5.route('/update_profile_picture', methods=['POST'])
@token_required
def update_profile_picture(active_user):
    try:
        ...
        if profile_pic:
            ...
            image_name = secure_filename(profile_pic.filename)
            extension = os.path.splitext(image_name)[1]
            extension2 = os.path.splitext(image_name)[1][1:].lower()
            ...
            s3_client.upload_fileobj(profile_pic, S3_BUCKET, image_name,
                                     ExtraArgs={'ACL': 'public-read', 'ContentType': content_type})
            image_url = f"https://{S3_BUCKET}.s3.amazonaws.com/{image_name}"

            active_user.image_name = image_name
            active_user.image_path = image_url

            db.session.commit()

            return jsonify({'status': 1,'messege': 'Profile picture updated successfully','image_url': image_url})
...
@user_auth_v5.route('/user_update', methods=['POST', 'GET'])
@token_required
def user_update(active_user):
    ...
    if request.method == 'POST':
        ...
        if user_image:
            ...
            user_data = User(user_bio=user_bio,profile_link = profile_link,new_bio = new_bio,college=college, about_me=about_me, description_box=description_box, id=active_user.id,
        ...
        if active_user.looking_for == 'Friends':
            looking_for = '0'
        elif active_user.looking_for == 'Dating':
            looking_for = '1'
        ...
        box_values = {
            ...
            "box_1": active_user.box_1 if active_user.box_1 is not None else '',
            "box_2": active_user.box_2 if active_user.box_2 is not None else '',
            "box_3": active_user.box_3 if active_user.box_3 is not None else '',
            "box_4": active_user.box_4 if active_user.box_4 is not None else '',
            "box_5": active_user.box_5 if active_user.box_5 is not None else '',
            "box_6": active_user.box_6 if active_user.box_6 is not None else '',
            "box_7": active_user.box_7 if active_user.box_7 is not None else '',
            "box_8": active_user.box_8 if active_user.box_8 is not None else '',
            "box_9": active_user.box_9 if active_user.box_9 is not None else '',
            "box_10": active_user.box_10 if active_user.box_10 is not None else ''
        ...
        users_data = {'id': str(active_user.id),
                     ...
                     'user_badge': active_user.user_badge if active_user.user_badge is not None else '',
            'user_bio': active_user.user_bio if active_user.user_bio is not None else ''
...
@user_auth_v5.route('/double_verification', methods=['POST'])
@token_required
...
@user_auth_v5.route("/user_to_user/block", methods=['POST'])
@token_required
...
@user_auth_v5.route("/block_list", methods=['GET'])
@token_required
...
@user_auth_v5.route("/user/report", methods=['POST'])
@token_required
...
@user_auth_v5.route("/user/forget_password", methods=['POST'])
...
@user_auth_v5.route("/user/new_password", methods=['POST'])
@token_required
...
@user_auth_v5.route("/subscription", methods=['POST'])
@token_required
...
@user_auth_v5.route('/add_image_s', methods=['GET', 'POST'])
def add_image_s():
